...
function generateManifest(podCount: number, lessonCount: number, moduleCount: number, domains: string[]) {
  const content = `// AUTO-GENERATED by sync-learning-content.ts — do not edit
...
export const manifest: ContentManifest = {
  ...
  domains: ${JSON.stringify(domains.sort())},
...
function main() {
  ...
  const domains = [...new Set(pods.map((p) => p.domain))];
  generateManifest(pods.length, lessons.length, modules.length, domains);
